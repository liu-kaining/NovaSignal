    ...
    async with semaphore:
        ...
        try:
            ...
            agent_instruction = (
                ...
                "Follow prompt.md exactly — it is the product contract. "
                "Before claiming any information gap, exhaustively analyze every non-null "
                "section of raw_data.json (ipo_data, company_profile, company_notes, financials, "
                "fundamental_extras, comparables, ownership_governance, sell_side, "
                "sector_industry_context, global_market_context, "
                "sec_filings_recent, ipo_regulatory_context, news_context, "
                "market_context including quote, fundraising). Check fetch_errors for "
                "upstream FMP misses. "
                "Where prompt.md still requires facts that are missing, you MUST use "
                "Claude Code web search / deep research tools to retrieve reputable "
                "sources (EDGAR / exchange filings, issuer IR, major financial press). "
                "Cite source titles and URLs in report.md (never invent links or numbers). "
                "Produce report.md and metrics.json as specified."
            )
